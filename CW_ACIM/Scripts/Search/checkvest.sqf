...
_customVar = _cursZiv getvariable "acim_custom_vestitem";
_alreadyAssignedItem = _cursZiv getVariable "acim_vest_searched";
_to_assign_item = "ERROR";
_timer = 20;
_vestitems = vestitems _cursZiv;
If (isNil "_vestitems") then {_vestitems = []};
_ACIM_INVENTORY_TEMP_USEDITEMNAMES_VEST = _cursZiv getVariable "ACIM_INVENTORY_TEMP_USEDITEMNAMES_VEST";
If (isNil "_ACIM_INVENTORY_TEMP_USEDITEMNAMES_VEST") then {_ACIM_INVENTORY_TEMP_USEDITEMNAMES_VEST = []};
...
_cursZiv setVariable ["acim_vest_searched", _to_assign_item, true]; //SET SEARCHED ITEM VARIABLE
...
while {_timer > 0} do
{
	If (player distance _cursZiv > 2) exitWith 			//IF PLAYER TOO FAR AWAY.....
	{
		...
		_cursZiv setVariable ["acim_vest_searched", nil]; //...RESET SEARCHED ITEM VARIABLE
	};
...
If (_timer == 0) then 
{
	If (isNil {_cursZiv getvariable "acim_vest_searched"}) then 
	{  
		["acim_vest_searched", _to_assign_item, true];
	};
	...
	If ({_x == _to_assign_item} count _ACIM_INVENTORY_TEMP_USEDITEMNAMES_VEST == 0) then 
	{
		If (_to_assign_item != (localize "STR_CW_ACIM_FINDS_Nothing")) then {ACIM_INVENTORY_TEMP_USEDITEMNAMES_VEST = ACIM_INVENTORY_TEMP_USEDITEMNAMES_VEST + [_to_assign_item];};
		
		_cursZiv setVariable ["ACIM_INVENTORY_TEMP_USEDITEMNAMES_VEST", _ACIM_INVENTORY_TEMP_USEDITEMNAMES_VEST + [_to_assign_item], true];
	
		If (_to_assign_item == (localize "STR_CW_ACIM_FINDS_Nothing")) then {_cursZiv setVariable ["ACIM_CIV_INVENTORY", _cursZivInvVar + _vestitems, true];};
		If (_to_assign_item != (localize "STR_CW_ACIM_FINDS_Nothing")) then {_cursZiv setVariable ["ACIM_CIV_INVENTORY", _cursZivInvVar + [_to_assign_item] + _vestitems, true];};
	};	
